...
namespace Origin
{
    ...
    {
        if (const FramebufferSpecification gameSpec = m_GameFramebuffer->GetSpecification();
            m_GameViewportSize.x > 0.0f && m_GameViewportSize.y > 0.0f &&
            (m_GameViewportSize.x != gameSpec.Width || m_GameViewportSize.y != gameSpec.Height))
        {
            m_GameFramebuffer->Resize(static_cast<uint32_t>(m_GameViewportSize.x),
                                      static_cast<uint32_t>(m_GameViewportSize.y));
            m_ActiveScene->OnGameViewportResize(static_cast<uint32_t>(m_GameViewportSize.x),
                                       static_cast<uint32_t>(m_GameViewportSize.y));
        }
    ...
    void Editor::OpenScene(const std::filesystem::path& path)
    {
        ...
        if (serializer.Deserialize(path.string()))
        {
            ...
            m_ActiveScene->OnGameViewportResize(
              static_cast<uint32_t>(m_GameFramebuffer->GetWidth()),
              static_cast<uint32_t>(m_GameFramebuffer->GetHeight()));
        }
    }
