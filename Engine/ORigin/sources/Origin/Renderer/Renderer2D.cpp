...
void Renderer2D::Init()
{
	...
	s_Render2DData.QuadVertexBuffer = VertexBuffer::Create(Renderer::render_data.max_vertices * sizeof(QuadVertex));
	...
	s_Render2DData.QuadVertexBufferBase = new QuadVertex[Renderer::render_data.max_vertices];

	uint32_t* quadIndices = new uint32_t[Renderer::render_data.max_quad_indices];

	uint32_t offset = 0;
	for (uint32_t i = 0; i < Renderer::render_data.max_quad_indices; i += 6)
	{
	...
	s_Render2DData.TextVertexBuffer = VertexBuffer::Create(Renderer::render_data.max_vertices * sizeof(TextVertex));
	...
	s_Render2DData.TextVertexBufferBase = new TextVertex[Renderer::render_data.max_vertices];

	// Circles
	s_Render2DData.CircleVertexArray = VertexArray::Create();
	s_Render2DData.CircleVertexBuffer = VertexBuffer::Create(Renderer::render_data.max_vertices * sizeof(CircleVertex));
	...
	s_Render2DData.CircleVertexBufferBase = new CircleVertex[Renderer::render_data.max_vertices];

	// Lines
	s_Render2DData.LineVertexArray = VertexArray::Create();
	s_Render2DData.LineVertexBuffer = VertexBuffer::Create(Renderer::render_data.max_vertices * sizeof(LineVertex));
...
void Renderer2D::DrawQuad(const glm::mat4& transform, const std::shared_ptr<Texture2D>& texture, const glm::vec2& tilingFactor, const glm::vec4& tintColor)
{
	...
	{
		if (s_Render2DData.TextureSlotIndex >= Renderer::render_data.max_texture_slots)
			NextBatch();
...
void Renderer2D::DrawString(const std::string& string, std::shared_ptr<Font> font, const glm::mat4& transform, const TextParams& textParams, glm::vec2 *size)
{
	...
	if (s_Render2DData.FontAtlasTextureIndex >= Renderer::render_data.max_texture_slots)
		NextBatch();
