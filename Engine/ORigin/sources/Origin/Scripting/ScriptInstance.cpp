...
namespace origin
{
    ...
    bool ScriptInstance::GetFieldValueInternal(const std::string &name, void *buffer)
    {
        ...
        if (field.Type == ScriptFieldType::Entity)
        {
            ...
            {
                PUSH_CONSOLE_ERROR("[Script Instance] Could not get field '{}' in class", name);
                OGN_CORE_ERROR("[Script Instance] Could not get field '{}' in class", name);
                return false;
            }
            ...
            if (!idField)
            {
                PUSH_CONSOLE_ERROR("[Script Instance] Failed to find field '{}' in Entity class", name);
                OGN_CORE_ERROR("[Script Instance] Failed to find field '{}' in Entity class", name);
                return false;
            }
    ...
    bool ScriptInstance::SetFieldValueInternal(const std::string &name, const void *value)
    {
        ...
        if (it == fields.end())
        {
            PUSH_CONSOLE_ERROR("[Script Instance] Failed to set field '{}' value", name);
            OGN_CORE_ERROR("[Script Instance] Failed to set field '{}' value", name);
            return false;
        }
