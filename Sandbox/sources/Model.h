...
namespace origin
{
    ...
    bool readSkeleton(TestBone &boneOutput, aiNode *node,
    ...
    void loadModel(const aiScene *scene, aiMesh *mesh, std::vector<Vertex> &verticesOutput,
        std::vector<uint32_t> &indicesOutput, TestBone &skeletonOutput, uint32_t &nBoneCount)
    {
        verticesOutput = {};
        indicesOutput = {};
    ...
    void LoadAnimation(const aiScene *scene, TestAnimation &animation)
    {
    ...
    void GetPose(TestAnimation &animation, TestBone &skeleton, float timeMS, std::vector<glm::mat4> &output, glm::mat4 &parentTransform, glm::mat4 &globalInverseTransform)
    {
        ...
        for (TestBone &child : skeleton.children)
        {
